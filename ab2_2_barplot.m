...
for mdl_ind=1:5
    ...
    Pa(:,7)=[Pa(1:7,1);Pa(1:7,2);Pa(1:7,3);Pa(1:7,4);Pa(1:7,5);Pa(1:7,6)]
    for mnd=1:42%MND12
        if mnd>3
            Ct(mnd,:)=NaN
        else
    ...
    Ct(:,7)=[Ct(1:7,1);Ct(1:7,2);Ct(1:7,3);Ct(1:7,4);Ct(1:7,5);Ct(1:7,6)]
    ...
    [t_face,p_face]=ttest2([Pa(:,1);Pa(:,2)],[Ct(:,1);Ct(:,2)],'Tail','left')
    [t_hand,p_hand]=ttest2([Pa(:,3);Pa(:,4)],[Ct(:,3);Ct(:,4)],'Tail','left')
    [t_leg,p_leg]=ttest2([Pa(:,5);Pa(:,6)],[Ct(:,5);Ct(:,6)],'Tail','left')
