...
function ab5_correlation_Ferritin_I_II_and_T2_Fc_Leg
...
for mdl_ind=1:5
    ...
    figure
    hold on
    scatter([T2_Fc,T2_ACC,T2_V2]',[Fe_Fc,Fe_ACC2,Fe_V22]','w')
    lsline
    
    scatter(T2_Fc,Fe_Fc,'ko')
    scatter(T2_Fc(end-2:end),Fe_Fc(end-2:end),'ro')
    
    scatter(T2_ACC,Fe_ACC2,'k+')
    scatter(T2_ACC(end-2:end),Fe_ACC2(end-2:end),'r+')
    
    scatter(T2_V2,Fe_V22,'k*')
    scatter(T2_V2(end-2:end),Fe_V22(end-2:end),'r*')
    
    [R,PValue] = corr([T2_Fc,T2_ACC,T2_V2]',[Fe_Fc,Fe_ACC2,Fe_V22]')
    title(strcat(mdl{mdl_ind},'(R=',num2str(R),';P=',num2str(PValue),')'))
    saveas(gcf,strcat('FinalScatter-Fc-ACC-V2',mdl{mdl_ind},'.jpg'))
