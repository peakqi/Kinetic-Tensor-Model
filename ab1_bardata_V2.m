function ab1_bardata_V2
addpath('/vols/Data/km/fengqi/NIfTI');

[durationFrame,MND_ID]=a_dataframe

temp=strcat(MND_ID,'_V2_MASK_L_clean.nii.gz');
temp=strcat('WorkingFolder/ROImasks/',temp);
m{1}=qf_load(temp);

temp=strcat(MND_ID,'_V2_MASK_R_clean.nii.gz');
temp=strcat('WorkingFolder/ROImasks/',temp);
m{2}=qf_load(temp);
...
save(strcat('../',MND_ID,'_V2_FinalBar.mat'),'tab','tab_v2','tab_v3','tab_v4','tab_v5')
